...
def learn_direction(x_data, y_label):
    ...
    attractive_direction = model.coef_.reshape((-1, w_dim))
...
def generate_image_with_w_plus(generator, w_plus, draw_bounding_box=False):
    w_dim = generator.w_dim
    w_broadcasted = np.reshape(w_plus, newshape=(1, -1, w_dim))
...
def move_n_save(g, latent_vector, direction, coefficients, output_fn):
    ...
    for r in range(n_rows):
        for c in range(n_cols):
            ...
            image = generate_image_with_w_plus(g, new_latent_vector, draw_bounding_box)
